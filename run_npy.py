...
classes = ['no_finding', 'suspicious_calcification', 'mass']
for i in classes:
    os.makedirs(os.path.join(out_folder, 'train', i), exist_ok=True)
    os.makedirs(os.path.join(out_folder, 'test', i), exist_ok=True)
        ...
        with tqdm(total=len(img_tensor), desc=f'Processing', leave=False) as pbar_inner:
            for jdx, (img, label) in enumerate(zip(img_tensor, label_tensor)):
                ...
                cv2.imwrite(os.path.join(
                    out_folder, 'train', classes[label], f'{idx}_{jdx}.png'), img[0])
                pbar_inner.update(1)
        pbar_outer.update(1)
    ...
    for idx, (image_file, label_file) in enumerate(zip(test_image_files, test_label_files)):
        ...
        with tqdm(total=len(img_tensor), desc=f'Processing', leave=False) as pbar_inner:
            for jdx, (img, label) in enumerate(zip(img_tensor, label_tensor)):
                ...
                cv2.imwrite(os.path.join(
                    out_folder, 'test', classes[label], f'{idx}_{jdx}.png'), img[0])
                pbar_inner.update(1)
        pbar_outer.update(1)
